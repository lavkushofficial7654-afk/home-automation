/*
 * ESP8266 Guest Network + Ultrasonic Auto Control
 * 
 * Features:
...
void setup() {
  ...
  Serial.println("=====================================");
  Serial.println("ESP8266 Guest Network + Auto Control");
  Serial.println("=====================================");
  ...
  // Setup web server routes (preserved for voice recognition)
...
// Web server handlers (preserved for voice recognition)
void handleRoot() {
  float distance = measureDistance();
  
  String html = "<!DOCTYPE html><html>";
  html += "<head><title>ESP8266 Auto Control</title>";
  ...
  html += ".mode { background: linear-gradient(45deg, #fff3e0, #ffe0b2); color: #e65100; padding: 15px; border-radius: 10px; margin: 15px 0; }";
  html += "h1 { color: #333; margin-bottom: 30px; font-size: 24px; }";
  html += "</style></head>";
  html += "<body>";
  html += "<div class='container'>";
  html += "<h1>🔌 Auto Control + 📏 Ultrasonic</h1>";
  ...
  html += "<br><br>";
  html += "<p style='color:#666; font-size:14px;'>🎮 Manual Control + 🔄 Auto ON</p>";
  html += "</div>";
  html += "<script>";
...
void handleLedOn() {
  ...
  ledState = true;
  manualOverride = true;  // User manually controlled
  Serial.println("LED turned ON via web (Manual)");
...
void handleLedOff() {
  ...
  ledState = false;
  manualOverride = true;  // User manually controlled
  Serial.println("LED turned OFF via web (Manual)");
...
void handleLedToggle() {
  ...
  manualOverride = true;  // User manually controlled
  Serial.println("LED toggled via web to: " + String(ledState ? "ON" : "OFF") + " (Manual)");
