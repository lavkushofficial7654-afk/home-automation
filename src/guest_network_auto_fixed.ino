/*
 * ESP8266 Guest Network + Ultrasonic Sensor (Auto Fixed)
 * 
 * Uses safe pin configuration to avoid conflicts
 * TRIG: D1 (GPIO5), ECHO: D2 (GPIO4)
 * Improved auto control logic
 */
...
void setup() {
  ...
  Serial.println("=====================================");
  Serial.println("ESP8266 Guest Network + Auto Fixed");
  Serial.println("=====================================");
  ...
  if (sensorWorking) {
    ...
    Serial.println("Auto control will start in 5 seconds...");
  } else {
...
void loop() {
  ...
  if (sensorWorking && millis() - lastAutoCheck >= AUTO_CHECK_INTERVAL) {
    ...
    if (!manualOverride) {  // Only auto control if no manual override
      ...
      // Debug: Print distance every few checks
      static int debugCounter = 0;
      debugCounter++;
      if (debugCounter % 5 == 0) {  // Print every 5 seconds
        Serial.print("Auto check - Distance: ");
  ...
  if (Serial.available()) {
    ...
    if (command == "on") {
      ...
      Serial.println(sensorWorking ? "Yes" : "No");
      Serial.print("Manual override: ");
      Serial.println(manualOverride ? "Yes" : "No");
      ...
      if (sensorWorking) {
        ...
        Serial.print(distance, 1);
        Serial.println(" cm");
        Serial.print("Should auto ON: ");
        Serial.println((distance > 0 && distance < TRIGGER_DISTANCE) ? "Yes" : "No");
      }
      Serial.println("==================");
    }
